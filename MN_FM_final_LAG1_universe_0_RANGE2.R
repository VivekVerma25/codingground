...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/OUTPUT2/"
...
fac_list = data.frame(read.csv(paste(path_input,"ANALYST ACTUAL DELTA250.csv",sep="")))
...
uni_list = data.frame(read.csv(paste(path_input,"UNIVERSE_AR.csv",sep="")))
...
for(z in 1 : dim(fac_list)[1])
{
  ...
  for(u in 1: dim(uni_list)[1])
  {
    ...
    f_factor[f_factor<0.00001 & f_factor>-0.00001]<-NA
