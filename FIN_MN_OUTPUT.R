...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/FINAL FILES/OUTPUT/"
...
raw_fut_close= data.frame(read.csv(paste(path_input,"FUT DB/","PX_LAST FUT-1.csv",sep="")))
raw_basis = raw_cash_close
raw_basis[,-1] = (raw_cash_close[,-1] - raw_fut_close[,-1])/(raw_cash_close[,-1])
list_basis <- vector('list',3)
...
for(z in 1 : length(fac_list))
{
  ...
  fac_1<-fac_list[[z]] # factor file input
    ...
    stapp = data.frame(read.csv(paste(path_input,"ACTIVE FO-N50.csv",sep="")))
    ...
    if(dsort!=1){
      ...
      if (topbot==2){       
        ...
        ret_fin_tb<-data.frame(cbind(stck_lvl$Returns[c(-1,-2)],tot_ret_tb,lng_ret_tb,sht_ret_tb,wts_ini_full_tb[c(-1,-1*dim(wts_ini_full_tb)[1]),]))
