...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/FINAL FILES/OUTPUT/PESTP NA50 DN/"
...
topbot<-2 # Make this 2 if you neeed top x and bottom x RANK weighted
xnum<-5  # This is the number of stocks you need to select both on the top side and bottom side
...
raw_est_eps= data.frame(read.csv(paste(path_input,"ANALYST DB/","BEST_ANALYST_RATING.csv",sep="")))

list_PEST <- vector('list',10)


#list_PEST <- vector('list',2)
#lookback_period=c(10,20)
lookback_period=c(5,10,20,40,60,90,120,150,180,240)
# 
# RAW_PEST<-raw_cash_close
# dt<-RAW_PEST[1:nrow(RAW_PEST),1]
# x = raw_est_eps[,-1]/raw_cash_close[,-1]
# x<-as.matrix(x)
...
for(l in 1:length(lookback_period)){
  ...
  wq1<-raw_est_eps[,-1]
  dt<-raw_est_eps[1:nrow(raw_est_eps),1]
  wq1[c(-1:-lookback_period[l]),] =wq1[c((-dim(wq1)[1]):(-dim(wq1)[1]+lookback_period[l]-1)),]
  ...
  wq2<-raw_cash_close[,-1]
  #dt<-raw_cash_close[1:nrow(raw_cash_close),1]
  wq2[c(-1:-lookback_period[l]),] =wq2[c((-dim(wq2)[1]):(-dim(wq2)[1]+lookback_period[l]-1)),]
  
  roll=  (1+((raw_est_eps[,-1]-wq1)/wq1))/(1+((raw_cash_close[,-1]-wq2)/wq2))
...
names(list_PEST) <- list("PEST5","PEST10","PEST20","PEST40","PEST60","PEST90","PEST120","PEST150","PEST180","PEST240")
#names(list_PEST) <- list("PEST10","PEST20")
#######################################################################################################################################################
