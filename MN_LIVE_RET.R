...
RETURNS_OP<-data.frame(matrix(0,NOS,1))
colnames(RETURNS_OP)<- stck_lvl[nrow(stck_lvl),1]
...
for(z in 1 : length(fac_list))
{
  ...
  if(dsort!=1){
    ...
    if (topbot==2){       
      ...
      RETURNS_OP[z,1] = tot_ret_tb[(dim(tot_ret_tb)[1]),1]
...
#RETURNS_FILE = data.frame((cbind(DATE,RETURNS_OP)))
rownames(RETURNS_OP) <- names(list_full)
...
#write.csv(t(FINAL_FILE),paste(path_output,"MN ALLOC_TEST.csv"))
write.csv(t(RETURNS_OP),paste(path_output,"MN THEO RETURNS 23-05-2017.csv"))
