...
################################################################################################################## UNIVERSE LIST
uni_list = data.frame(read.csv(paste(path_input,"UNIVERSE LIST.csv",sep="")))
...
fac_list = data.frame(read.csv(paste(path_input,"final analyst factors sign.csv",sep="")))

fac_list$dir = as.character(fac_list$dir)
fac_list$files = as.character(fac_list$files)
fac_row <-  dim(fac_list)[1]
...
for(z in 1 : dim(fac_list)[1])
{
  ...
  colnames(ret_full_cmbn)[z+(u-1)*(fac_row)+1]= paste(fac_list[z,2],"_",uni_list[u,2])
  
  
  
  if (topbot==1){   
    ret_tb_cmbn = data.frame(cbind(ret_tb_cmbn,tot_ret_tb))
    colnames(ret_tb_cmbn)[z+(u-1)*(fac_row)+1]= paste(fac_list[z,2],"_",uni_list[u,2])
