...
topbot<-0 # Make this 1 if you neeed top x and bottom x equay weighted
...
clafic<-data.frame(read.csv(paste(path_input,"Sectors new.csv",sep="")))[,-1] 
...
n<-max(clafic)                                # This signifies the numbers of sectors etc
  ...
  fac_1<-data.frame(read.csv(paste(path_input,"ROE.csv",sep=""))) # factor file input
  ...
  if(dsort!=1){
    ...
    for (i in 1:n){
      ...
      fac_clas_sel_2<-(fac_clas_sel_1-rowMeans(fac_clas_sel_1,na.rm=TRUE))
      #/rowSums(abs(fac_clas_sel_1-rowMeans(fac_clas_sel_1,na.rm=TRUE)),na.rm=TRUE)
    ...
    wts_fin<- wts_fin/rowSums(abs(wts_fin),na.rm=TRUE)
    ...
    write.csv(ret_fin_full,paste(path_output,"ROE_SEC_NEW.csv"),row.names=FALSE)
    ...
    write.csv(ret_fin_tb,paste(path_output,"EPS_DELTA20_z_tb.csv"))
    }
