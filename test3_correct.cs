...
{
    public class test3_correct : BasicStrategy
    {
        ...
        public test3_correct(string stratName, double alloc, double cost, double timeStep)
                            ...
                            double metric = z2/z1;
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    if (currentmove1< 0)
                                    {
                                        ...
                                        if (z1 >= siglevel1 && np[timestep - 1] != 1 && metric <= sigdiffL && (mode == "A" || mode == "L") && longtrades < LC && flag <= 0 && ltp_stock[timestep] >= 20.0)
                                        {
                                            sig[timestep] = +2;
                                            np[timestep] = +1;
                                            timeintrade = 0;
