...
{
    public class AD_range : BasicStrategy
    {
        ...
        public AD_range(string stratName, double alloc, double cost, double timeStep)
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime)
                    {
                        ...
                        {
                            if (diff1 > adm*newseries2.Average() && longflag == true)
                            {
                                sig[j] = +2;
                                np[j] = +1;
                            }

                            if (diff1 < -adm*newseries2.Average() && shortflag == true)
                            {
                                sig[j] = -2;
                                np[j] = -1;
                            }
                    ...
                    if ((np[j - 1] == 1 && diff1 < -adm*0.5 * newseries2.Average()) || (np[j - 1] == -1 && diff1 > adm *0.5* newseries2.Average()))
                    {
                        sig[j] = -np[j - 1];
                        np[j] = 0;
                    }
