...
{
    public class ADRatio_disp_dyn1 : BasicStrategy
    {
        ...
        public ADRatio_disp_dyn1(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    //dist2 = dist2 + Math.Abs(ad[j] - openad2);
                    if (timecounter > 3)
                    {
                        dist1 += Math.Abs(ad[j] - openad);
                    }
                    ...
                    {
                        if ((diff1 / dist1) > dthresh * (Math.Min((75 / (timecounter)) / 40, 0.05)) && longflag == true)
                        {
                            sig[j] = +2;
                            np[j] = +1;
                            //openad2 = ad[j];
                        ...
                        if ((diff1 / dist1) < -dthresh * (Math.Min((75 / (timecounter)) / 40, 0.05)) && shortflag == true)
                        {
                            sig[j] = -2;
                            np[j] = -1;
                            // openad2 = ad[j];
                    ...
                    if ((np[j - 1] == 1 && (diff2 / dist2) < adsqm) || (np[j - 1] == -1 && (diff2 / dist2) > -adsqm))
                    {
                        sig[j] = -np[j - 1];
                        np[j] = 0;
                    }
