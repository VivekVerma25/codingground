...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/ANALYST FINAL TEST2/"
...
fac_list = data.frame(read.csv(paste(path_input,"FIN ANA FAC.csv",sep="")))
...
uni_list = data.frame(read.csv(paste(path_input,"ANALYST UNI.csv",sep="")))
...
for(z in 1 : dim(fac_list)[1])
{
  ...
  for(u in 1: dim(uni_list)[1])
  {
    ...
    if(dsort!=1){
      ...
      tot_ret_full<-data.frame(((lng_ret_full+sht_ret_full*(lng_beta_full/sht_beta_full)))-cst_full)
      ...
      if (topbot==2){       
        ...
        tot_ret_tb<-data.frame(((lng_ret_tb+sht_ret_tb*(lng_beta_tb/sht_beta_tb))) - cst_tb)
