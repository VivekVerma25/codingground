...
{
    public class RSI : BasicStrategy
    {
        ...
        public object PriceCutoff = 500;

        public RSI(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                double[] nifty = data.InputData[i].Prices;
                ...
                for (int j = 1; j < nifty.Length; j++)
                {
                    //bar[j] = nifty[j]/nifty[j - 1] - 1;

                    bar[j] = nifty[j] - nifty[j - 1];
                    ...
                    if (j > tmaP)
                    {
                        ...
                        if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && nifty[j] >= pco && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime && RSI[j] <= thresh && np[j - 1] != -1 && shortctr <= 100)
                        {
                            sig[j] = -2;
                            np[j] = -1;
                            shortctr++;
                        ...
                        if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && nifty[j] >= pco && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime && RSI[j] >= 100 - thresh && np[j - 1] != 1 && longctr <= 100)
                        {
                            sig[j] = 2;
                            np[j] = 1;
                            longctr++;
