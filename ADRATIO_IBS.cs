...
{
    public class ADRATIO_IBS : BasicStrategy
    {
        ...
        public ADRATIO_IBS(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            double ibs = Convert.ToDouble(IBS);
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                double ibss = 100000;
                double diff2 = -100;
               // double hi = 99999999999999999; 
                //double lw = -9999999999999999;
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
...

                    {
                        ...
                        ibss = (ltp[j] - Move1.Min()) / (Move1.Max() - Move1.Min());
                    ...
                    {
                        if (diff1 > adm && ibss <ibs && longflag == true )
                        {
                            sig[j] = +2;
                            np[j] = +1;
                        }

                        if (diff1 < -adm && ibss >(1-ibs) && shortflag == true)
                        {
                            sig[j] = -2;
                            np[j] = -1;
                        }
