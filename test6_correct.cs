...
{
    public class test6_correct : BasicStrategy
    {
        ...
        public test6_correct(string stratName, double alloc, double cost, double timeStep)
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                    ...
                    if (timestep - lbk > 0 && data.InputData[i].Dates[timestep].Date == data.InputData[i].Dates[timestep - lbk].Date)
                    {
                        ...
                        if (data.InputData[i].Dates[timestep].TimeOfDay >= DataRefStartTime && data.InputData[i].Dates[timestep].TimeOfDay <= DataRefEndTime)
                        {
                            ...
                            double[] currentmove2 = new double[lbk / 5];

                            for (int j = 2; j < lbk / 5; j++)
                            {
                                currentmove1[j] = Math.Log(ltp_stock[timestep] / ltp_stock[timestep - 5 * (j + 1)]);
                                currentmove2[j] = Math.Log(ltp_sec[timestep] / ltp_sec[timestep - 5 * (j + 1)]);
                            }


                            Move1.Add(Math.Abs(currentmove1[(lbk / 5) - 1]));
                            Move2.Add(Math.Abs(currentmove2[(lbk / 5) - 1]));
                            ...
                            if (series1.Length > lbk2 && series2.Length > lbk2)
                            {
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    if (currentmove1[z1_max_i] < 0)
                                    {
                                        ...
                                        if (z1[z1_max_i] >= siglevel1 && metric<=z3.Min() && np[timestep - 1] != 1 && metric <= sigdiffL && (mode == "A" || mode == "L") && longtrades < LC && flag <= 0 && ltp_stock[timestep] >= 20.0)
                                        {
                                            sig[timestep] = +2;
                                            np[timestep] = +1;
                                            timeintrade = 0;
                    ...
                    if (((timeintrade >= 5*(zzz + 1) && timeintrade >= et1) || timeintrade >= et2) && np[timestep - 1] != 0)
                    {
