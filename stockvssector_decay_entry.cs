...
{
    public class stockvssector_decay_entry : BasicStrategy
    {
        ...
        public stockvssector_decay_entry(string stratName, double alloc, double cost, double timeStep)
                    ...
                    if (timestep - lbk > 0 && data.InputData[i].Dates[timestep].Date == data.InputData[i].Dates[timestep - lbk].Date)
                    {
                        ...
                        if (series1.Length > lbk2 && series2.Length > lbk2)
                        {
                            ...
                            if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                            {
                                ...
                                { 
                                    double[] currentmove3 = new double[lbk3];
                                    double[] currentmove4 = new double[lbk3];
                                    double[] z3 = new double[lbk3];
                                    double[] z4 = new double[lbk3];


                                     for (int j = 0; j < lbk3; j++)
                                    {
                                        currentmove3[j] = Math.Abs(Math.Log(ltp_stock[timestep-j-1] / ltp_stock[timestep - j-1 - lbk]));
                                        currentmove4[j] = Math.Abs(Math.Log(ltp_sec[timestep - j - 1] / ltp_sec[timestep - j - 1 - lbk]));
                                        z3[j] = ((currentmove4[j] - avg2) / std2)/((currentmove3[j] - avg1) / std1);
                                        //z4[j] = (currentmove4[j] - avg2)/std2;
                                    }
                                   ...
                                   double ss = 0;
                                     ...
                                     if (z1[z1_max_i] >= siglevel1 && metric < w1 && np[timestep - 1] != 1 && metric <= sigdiffL && (mode == "A" || mode == "L") && longtrades < LC && flag <= 0)
                                {
                                    sig[timestep] = +2;
                                    np[timestep] = +1;
                                    timeintrade = 0;
