path_input<-"D:/LFT/Template/tester/R test data/full simulation/data files/BLENDED EST EPSQ/"
path_output<-"D:/LFT/Template/tester/R test data/full simulation/data files/BLENDED EST EPSQ/"

ff<-list.files(path_input)

checkdim =0
lookback_period=c(10,20,40,60)
for(l in 1:length(lookback_period)){
  
  for(i in 1:length(ff)){
  wq1<-read.csv(paste(path_input,ff[i],sep = ""),header = TRUE)
  dt<-wq1[1:nrow(wq1),1]
  roll<-rollapply(wq1[,2],lookback_period[l],mean,fill=NA,align = "right")
