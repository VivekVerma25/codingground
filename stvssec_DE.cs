...
{
    public class stvssec_DE : BasicStrategy
    {
        ...
        public object ExitTime2 = 12;
        public object LongCount = 100;
        public object NoTrade = 3;
        public object ExitRatio = 0.8;

        public stvssec_DE(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            double ER = Convert.ToDouble(ExitRatio);
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                List<double> zzzz = new List<double>(); 
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                        ...
                        if (series1.Length > lbk2 && series2.Length > lbk2)
                        {
                            ...
                            if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                            {
                                ...
                                if (currentmove1[z1_max_i] < 0)
                                {
                                    ...
                                    if (z1[z1_max_i] >= siglevel1 && metric <=z3.Min() && np[timestep - 1] != 1 && metric <= sigdiffL && (mode == "A" || mode == "L") && longtrades < LC && flag <= 0)
                                    {
                                        sig[timestep] = +2;
                                        np[timestep] = +1;
                                        timeintrade = 0;
                                        longtrades++;
                                        zz = z1[z1_max_i];
                                        cm1 = currentmove1[z1_max_i];
                                        cm2 = currentmove2[z1_max_i];
                                        zzzz=new List<double>(); 
                                    }
                   ...
                   /* if (timeintrade >= Math.Max((z1_max_i + 1), et) && np[timestep - 1] != 0)
                    ...
                    if (timeintrade >= et2 && np[timestep - 1] != 0)
                    {
                        sig[timestep] = -np[timestep - 1];
                        np[timestep] = 0;
                        timeintrade = 0;
                    ...
                    if (timeintrade >=et1 && np[timestep - 1] != 0)
                    {

                        double ExitMove1 = (Math.Log(ltp_stock[timestep] / ltp_stock[timestep - timeintrade]));
                        double ExitMove2 = (Math.Log(ltp_sec[timestep] / ltp_sec[timestep - timeintrade]));

                        double zz1 = (ExitMove1 - avg1)/std1;
                        double zz2 = (ExitMove2 - avg2)/std2;

                        zzzz.Add(zz2 / zz1);

                        if (zzzz.Count >= 2)
                        {
                            if (zzzz.Last() < zzzz[zzzz.Count - 2])
                            {
                                sig[timestep] = -np[timestep - 1];
                                np[timestep] = 0;
                                timeintrade = 0;
