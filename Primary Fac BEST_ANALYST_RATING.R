...
for(li in 1:length(lookback_period))
{
  ...
  ff<-c("ratio Total Buy - Total Sell to Total Analyst Reco.csv","PX_LAST.csv","BEST_EPS.csv","BEST_TARGET_PRICE.csv","BEST_EBITDA.csv") 
  for(f1 in 1:length(ff)){
    
   
    pric1<- read.csv(paste(path_input,"BEST_ANALYST_RATING.csv",sep=""))
    pric2<-read.csv(paste(path_input,ff[f1],sep=""))
    clafic<-data.frame(read.csv(paste(path_input,"TOP 100 ANALYST.csv",sep="")))[,-1]
    stck_lvl<-data.frame(read.csv(paste(path_input,"Roll adjusted returns.csv",sep="")))
    
    stck_lvl$Returns<-as.Date(stck_lvl$Returns,"%d/%m/%Y")
    names<-colnames(pric1)
    dt<-pric1[(lookback_period[li]+1):nrow(pric1),1]
    pric1<-pric1[,-1]
    test1<-pric1[(lookback_period[li]+1):nrow(pric1),]
    
    for(j in 1:ncol(pric1)){
      if(j==1) {
        roll<-rollapply(pric1[,j],lookback_period[li],mean,na.rm=TRUE,fill=NA,align = "right")
        m<-data.frame(roll)
      }
      else{
        roll<-rollapply(pric1[,j],lookback_period[li],mean,na.rm=TRUE,fill=NA,align = "right")
    ...
    for(j in 1:ncol(pric1)){
      if(j==1) {
        roll<-rollapply(pric1[,j],lookback_period[li],sd,na.rm=TRUE,fill=NA,align = "right")
        s<-data.frame(roll)
      }
      else{
        roll<-rollapply(pric1[,j],lookback_period[li],sd,na.rm=TRUE,fill=NA,align = "right")
    ...
    s<-s[lookback_period[li]:(nrow(s)-1),]
    m<-m[lookback_period[li]:(nrow(m)-1),]
    
    pcr<-pric1[(lookback_period[li]+1):nrow(pric1),]
    ...
    pric1<-zpcr
    fin<- matrix(0,dim(pric1)[1],dim(pric1)[2])
    pric1[pric1==0] <- NA
    ...
    dt<-pric2[(lookback_period[li]+1):nrow(pric2),1]
    pric2<-pric2[,-1]
    test2<-pric2[(lookback_period[li]+1):nrow(pric2),]
    
    for(j in 1:ncol(pric2)){
      if(j==1) {
        roll<-rollapply(pric2[,j],lookback_period[li],mean,na.rm=TRUE,fill=NA,align = "right")
        m<-data.frame(roll)
      }
      else{
        roll<-rollapply(pric2[,j],lookback_period[li],mean,na.rm=TRUE,fill=NA,align = "right")
    ...
    for(j in 1:ncol(pric2)){
      if(j==1) {
        roll<-rollapply(pric2[,j],lookback_period[li],sd,na.rm=TRUE,fill=NA,align = "right")
        s<-data.frame(roll)
      }
      else{
        roll<-rollapply(pric2[,j],lookback_period[li],sd,na.rm=TRUE,fill=NA,align = "right")
    ...
    m[is.na(m)] =0
    m[which(!is.finite(as.matrix(m)))] <- 0
    st2<-m[(lookback_period[li]):(nrow(m)-1),]
    
    s<-s[lookback_period[li]:(nrow(s)-1),]
    m<-m[lookback_period[li]:(nrow(m)-1),]
    
    pcr<-pric2[(lookback_period[li]+1):nrow(pric2),]
    ...
    pric2<-zpcr
    fin<- matrix(0,dim(pric2)[1],dim(pric2)[2])
    pric2[pric2==0] <- NA
    clafic<-clafic[(lookback_period[li]+1):nrow(clafic),]
    pric2[clafic==0]<-NA
    ...
    fin[2,which(as.matrix(pric1[1,])>1 & as.matrix(pric2[1,])>0)]<-1
    fin[3,which(as.matrix(pric1[2,])>1 & as.matrix(pric2[2,])>0)]<-1
    for(i in 4:nrow(fin)){
      fin[i,which(fin[i-1,]==0 & fin[i-2,]==0 & as.matrix(pric1[i-1,])>1 & as.matrix(pric2[i-1,])>0)]<-1
    ...
    for(i in 2:nrow(testprice)){
      if(sum(entry_l[i-1,]==1 | entry_s[i-1,]==-1)>0){
        ...
        while(j!=0){
          ...
          if(test1[i,c[j]]<st1[i,c[j]]){ #exit criteria
            entry_l[i,c[j]]=0
            netp=netp-1
          }
          else{
      ...
      if((sum(fin[i,]==1 | fin[i,]==-1)+netp)>=cs) {
        ...
        netp<-netp+(sum(fin[i,]==1)) +(sum(fin[i,]==-1))
      }
    ...
    if(fa==1){
      ...
      write.csv(final_res[1:8,],paste("C:/Users/YashV/Desktop/Framework Data/Results/","ANA_RATING ",ff[f1],lookback_period[li] ,"ANALYST_1sig_0sig_mean_l1.csv"),row.names=FALSE)
