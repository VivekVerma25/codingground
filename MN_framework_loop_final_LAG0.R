...
tc<-(0.0)                                # This is one side trnsaction cost 
...
fac_list = data.frame(read.csv(paste(path_input,"options factors.csv",sep="")))
...
for(z in 1 : dim(fac_list)[1])
{
  ...
  if(dsort!=1){
    ...
    ret_fin_full<-data.frame(cbind(stck_lvl$Returns[-1],tot_ret_full,lng_ret_full,sht_ret_full,wts_int_full[-1,]))
    ...
    if (topbot==1){       
      ...
      ret_fin_tb<-data.frame(cbind(stck_lvl$Returns[-1],tot_ret_tb,lng_ret_tb,sht_ret_tb,topname[-1,],botname[-1,]))
