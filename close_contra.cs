...
namespace StrategyCollection
{
    public class close_contra : BasicStrategy
    {
        ...
        public close_contra(string stratName, double alloc, double cost, double timeStep)
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                    ...
                    if (data.InputData[i].Dates[timestep].Date != data.InputData[i].Dates[timestep - 1].Date)
                    {
                        ...
                        {
                            ret1.Add((Math.Log(ltp_stock[timestep - 1] / lw)));

                        }

                        if (close.Count() > 0 && high.Count() > 0)
                        {
                            ret2.Add((Math.Log(ltp_stock[timestep - 1] / hi)));
                        ...
                        if (data.InputData[i].Dates[timestep].TimeOfDay >= DataRefStartTime && data.InputData[i].Dates[timestep].TimeOfDay <= DataRefEndTime)
                        {
                            ...
                            if (series2.Length > lbk2)
                            {
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    double z1 = ((Math.Log(ltp_stock[timestep]/lw)) - avg1) / std1;
                                    double z2 = ((Math.Log(ltp_stock[timestep]/hi)) - avg3) / std3;
                                    ...
                                    if (z1 >= siglevel1 && /*ltp_stock[timestep] > close.Last() && (Math.Log(ltp_stock[timestep] / lw) > 0.0) && z2 <= siglevel2 &&*/ np[timestep - 1] != -1 && (mode == "A" || mode == "L") && shorttrades < SC && flagS <= 0 && ltp_stock[timestep] > 20.0 && tt < lbk3)
                                    {
                                    ...
                                    if (z2 <= siglevel2 && /*ltp_stock[timestep] < close.Last() && (Math.Log(ltp_stock[timestep] / hi) < 0.0) && z1 <= siglevel1 &&*/ np[timestep - 1] != 1 && (mode == "A" || mode == "S") && longtrades < LC && flagL <= 0 && ltp_stock[timestep] > 20.0 && tt < lbk3)
                                    {
