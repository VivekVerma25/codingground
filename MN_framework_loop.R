...
fac_list = data.frame(read.csv(paste(path_input,"FACTOR LIST.csv",sep="")))

fac_list$NAME = as.character(fac_list$NAME)

ret_full_cmbn = data.frame(stck_lvl$Returns[-1])
...
fac_1<-data.frame(read.csv(paste(path_input,fac_list[z,1],".csv",sep=""))) # factor file input


f_factor<-data.frame(fac_1)                                 # This will be the final clculate foactor to be used for ranking
f_factor<-(f_factor[,-1]*fac_list[z,2])
...
colnames(ret_full_cmbn)[z+1]= fac_list[z,1]

ret_tb_cmbn = data.frame(cbind(ret_tb_cmbn,tot_ret_tb))
colnames(ret_tb_cmbn)[z+1]= fac_list[z,1]
}
