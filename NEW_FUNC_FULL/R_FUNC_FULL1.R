...
ROLL_STOCH = function(XX,N,NF,NS,NSIG,C){
  ...
  if(C==1)
  {
    ...
    roll = data.frame(apply(wq1[,-1],nFastK = N, nFastD = NF, nSlowD = NS, 2, stoch,percent = TRUE))[1:nrow(XX),]
  }
  
  if(C==2)
  {
    
    roll = data.frame(apply(wq1[,-1],nFastK = N, nFastD = NF, nSlowD = NS, 2, stoch,percent = TRUE))[(nrow(XX)+1):(2*nrow(XX)),]
  }
  ...
  if(C==3)
  {
    ...
    roll = data.frame(apply(wq1[,-1],nFastK = N, nFastD = NF, nSlowD = NS, 2, stoch,percent = TRUE))[(2*nrow(XX)+1):(3*nrow(XX)),]
  }
...
GROUP_MEAN = (XX,GRP){
  ...
  if (grouping == 1){
    ...
    group = data.frame(read.csv(paste(path_input,"UNIVERSE/","CUSTOM L3.csv",sep="")))[,-1]
  ...
  if (grouping == 2) {
    ...
    group = data.frame(read.csv(paste(path_input,"UNIVERSE/","CUSTOM L2.csv",sep="")))[,-1]
  ...
  if (grouping == 3) {
    ...
    group = data.frame(read.csv(paste(path_input,"UNIVERSE/","CUSTOM L1.csv",sep="")))[,-1]
    
  }
  
  if (grouping == 4) {
    
    group = data.frame(read.csv(paste(path_input,"UNIVERSE/","GICS L3.csv",sep="")))[,-1]
  ...
  if (grouping == 5) {
    ...
    group = data.frame(read.csv(paste(path_input,"UNIVERSE/","GICS L2.csv",sep="")))[,-1]
  ...
  if (grouping == 6) {
    ...
    group = data.frame(read.csv(paste(path_input,"UNIVERSE/","GICS L1.csv",sep="")))[,-1]
  ...
  for (i in 1:nn){
    ...
    # {
    #   
      fac_clas_sel_2<-as.matrix((rowMeans(fac_clas_sel_1,na.rm=TRUE)))
  ...
  if(i>1)
  {
    ...
    wts_fin<- as.matrix(cbind(wts_fin,data.frame(fac_clas_sell)))
    
    wts_fin = as.matrix(wts_fin/rowSums(abs(wts_fin),na.rm=TRUE))
    
    
    ############################################################################ Order Here
    # list_names = colnames(factor_rnk[1])
    
    wts_fin = wts_fin[,colnames(fac_clas)]
    ########################################################################### Ordering ends  
