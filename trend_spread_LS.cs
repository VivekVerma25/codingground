...
{
    public class trend_spread_LS : BasicStrategy
    {
        ...
        public object ExitTime1 = 60;
        public object ExitTime2 = 60;
        public object LongCount = 1;
        public object ShortCount = 1;
        public object NoTrade = 0;
        //public object returns = 0.000;

        public trend_spread_LS(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                        ...
                        if (data.InputData[i].Dates[timestep].TimeOfDay >= DataRefStartTime && data.InputData[i].Dates[timestep].TimeOfDay <= DataRefEndTime)
                        {
                            ...
                            if (series1.Length > lbk2)
                            {
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    if (z1 >= siglevel1 && np[timestep - 1] != -1 && (mode == "A" || mode == "S") && flag <= 0 && currentmove1 < 0.0 && ltp_stock[timestep] >= 20.0 && shorttrades <= SC)
                                    {
                                    ...
                                    if (z1 >= siglevel1 && np[timestep - 1] != 1 && (mode == "A" || mode == "L") && flag <= 0 && currentmove1 > 0.0 && ltp_stock[timestep] >= 20.0 && longtrades <= LC)
                                    {
                    ...
                    if (np[timestep] == 0 && np[timestep - 1] != 0)
                    {
                        flag = NT;
                    }
                    else
