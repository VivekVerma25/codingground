...
{
    public class RSI_AD : BasicStrategy
    {
        ...
        public RSI_AD(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = 1; j < (nifty.Length-lag); j++)
                {
                    ...
                    if (j > tmaP)
                    {
                        ...
                        if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime && RSI[j] <= thresh && np[j - 1+lag] != -1 && shortctr <= 100)
                        {
                            sig[j+lag] = -2;
                            np[j+lag] = -1;
                            shortctr++;
                        ...
                        if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime && RSI[j] >= 100 - thresh && np[j - 1+lag] != 1 && longctr <= 100)
                        {
                            sig[j+lag] = 2;
                            np[j+lag] = 1;
                            longctr++;
