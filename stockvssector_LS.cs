...
{
    public class stockvssector_LS : BasicStrategy
    {
        ...
        public stockvssector_LS(string stratName, double alloc, double cost, double timeStep)
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                    ...
                    if (timestep - lbk > 0 && data.InputData[i].Dates[timestep].Date == data.InputData[i].Dates[timestep - lbk].Date)
                    {
                        ...
                        if (series1.Length > lbk2 && series2.Length > lbk2)
                        {
                            ...
                            double metric1 = z2[z1_min_i] / z1[z1_min_i];
                            double metric2 = z2[z1_max_i] / z1[z1_max_i];
                            ...
                            if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                            {
                                ...
                                if (z1[z1_min_i] <= -siglevel1 && np[timestep - 1] != 1 && metric1 <= sigdiffL && (mode == "A" || mode == "L") && longtrades < LC)
                                {
                                    sig[timestep] = +2;
                                    np[timestep] = +1;
                                    timeintrade = 0;
                                ...
                                if (z1[z1_max_i] >= siglevel2 && np[timestep - 1] != -1 && metric2 <= sigdiffS && (mode == "A" || mode == "S") && shorttrades < SC)
                                {
                                    sig[timestep] = -2;
                                    np[timestep] = -1;
                                    timeintrade = 0;
                    ...
                    if (timeintrade >= Math.Max((z1_min_i + 1), et) && np[timestep - 1] != 0 && mode == "L")
                    {
                    ...
                    if (timeintrade >= Math.Max((z1_max_i + 1), et) && np[timestep - 1] != 0 && mode == "S")
                    {
                        sig[timestep] = -np[timestep - 1];
                        np[timestep] = 0;
                        timeintrade = 0;
