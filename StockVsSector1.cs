...
{
    public class StockVsSector1 : BasicStrategy
    {
        ...
        public StockVsSector1(string stratName, double alloc, double cost, double timeStep)
                    ...
                    if (timestep - lbk > 0 && data.InputData[i].Dates[timestep].Date == data.InputData[i].Dates[timestep - lbk].Date)
                    {
                        ...
                        if (series1.Length > lbk2 && series2.Length > lbk2)
                        {
                            ...
                            if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                            {
                                ...
                                if (z2 <= -siglevel1 && np[timestep - 1] != 1 && metric >= sigdiff && (mode == "A" || mode == "L"))
                                {
                                    sig[timestep] = +2;
                                    np[timestep] = +1;
                                    timeintrade = 0;
                                ...
                                if (z2 >= siglevel1 && np[timestep - 1] != -1 && metric >= sigdiff && (mode == "A" || mode == "S"))
                                {
                                    sig[timestep] = -2;
                                    np[timestep] = -1;
                                    timeintrade = 0;
