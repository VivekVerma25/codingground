...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/FINAL FILES/OUTPUT/PMN123 WEIGHTS/"
...
#######################################################################################################################################################
names(list_full) <- list("PMN01","PMN02","PMN03")
...
for(z in 1 : length(fac_list))
{
  ...
  if(z <4)
  {
    stapp = data.frame(read.csv(paste(path_input,"UNIVERSE/","ALLFO-N50.csv",sep="")))
