...
{
    public class low_contra_ad : BasicStrategy
    {
        public object TradeStartTime = 9.75;
        public object TradeEndTime = 14;
        public object TradeSquareOffTime = 15.75;
        public object RefStartTime = 9.5;
        public object RefEndTime = 16;
        public object Lookback = 45;
        public object Lookback2 = 20;
        public object Lookback3 = 48;
        ...
        public low_contra_ad(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                List<double> ret2 = new List<double>();
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                    ...
                    if (data.InputData[i].Dates[timestep].Date != data.InputData[i].Dates[timestep - 1].Date)
                    {
                        ...
                        if (close.Count() > 0 && low.Count() > 0)
                        {
                            ret1.Add(Math.Log(close.Last() / lw));

                        }

                        if (close.Count() > 0 && high.Count() > 0)
                        {
                            ret2.Add(Math.Log(close.Last() / hi));

                        }


                        open.Add(ltp_sec[timestep]);
                        close.Add(ltp_sec[timestep - 1]);
                        low = new List<double>();
                        low.Add(ltp_sec[timestep]);

                        high = new List<double>();
                        high.Add(ltp_sec[timestep]);
                        ...
                        if (ret1.Count() > lbk && close.Count() > lbk2 && ret2.Count() > lbk)
                        {
                            series1 = ret1.ToArray();
                            newseries1 = UF.GetRange(series1, series1.Length - lbk, series1.Length - 1);

                            series3 = ret2.ToArray();
                            newseries3 = UF.GetRange(series3, series3.Length - lbk, series3.Length - 1);
                        ...
                        if (data.InputData[i].Dates[timestep].TimeOfDay >= DataRefStartTime && data.InputData[i].Dates[timestep].TimeOfDay <= DataRefEndTime)
                        {
                            ...
                            if (series2.Length > lbk2)
                            {
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    double z1 = (Math.Log(close.Last() / ltp_sec[timestep]) - avg1) / std1;
                                    double z2 = (Math.Log(close.Last() / ltp_sec[timestep]) - avg3) / std3;
                                    ...
                                    if (z1 >= siglevel1 && /*ltp_stock[timestep] > close.Last() && (Math.Log(ltp_stock[timestep] / lw) > 0.0) && z2 <= siglevel2 &&*/ np[timestep - 1] != -1 && (mode == "A" || mode == "S") && longtrades < LC && flagS <= 0)
                                    {
                                    ...
                                    if (z2 <= -siglevel1 && /*ltp_stock[timestep] < close.Last() && (Math.Log(ltp_stock[timestep] / hi) < 0.0) && z1 <= siglevel1 &&*/ np[timestep - 1] != 1 && (mode == "A" || mode == "L") && shorttrades < SC && flagL <= 0)
                                    {
                    ...
                    if (timeintradeL >= et1 && np[timestep - 1] == +1)
                    {
                        sig[timestep] = -np[timestep - 1];
                        np[timestep] = 0;
                        timeintradeL = 0;
