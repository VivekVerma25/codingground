...
{
    public class trend_spread_LS1 : BasicStrategy
    {
        ...
        public object Lookback = 60;
        public object Lookback2 = 2000;
        ...
        public object SigmaLevel1 = 2.5;
        public object SigmaLevel2 = 1;
        public object ExitTime1 = 120;
        public object ExitTime2 = 120;
        public object LongCount = 1;
        public object ShortCount = 1;
        public object NoTrade = 0;
        public object Delay = 0;
      
        //public object NoTradeL = 0;
        //public object returns = 0.000;

        public trend_spread_LS1(string stratName, double alloc, double cost, double timeStep)
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                    ...
                    if (timestep - lbk > 0 && data.InputData[i].Dates[timestep].Date == data.InputData[i].Dates[timestep - lbk].Date)
                    {
                        ...
                        if (data.InputData[i].Dates[timestep].TimeOfDay >= DataRefStartTime && data.InputData[i].Dates[timestep].TimeOfDay <= DataRefEndTime)
                        {
                            ...
                            double currentmove1 = Math.Log(ltp_sec[timestep] / ltp_sec[timestep - lbk]);
                            ...
                            if (series1.Length > lbk2)
                            {
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    if (flag1 == 0)
                                    {
                                        if (Zscore2[timestep - DL] <= -siglevel2)
                                        {
                                    ...
                                    if (z1 >= siglevel1 && np[timestep] != -1 && (mode == "A" || mode == "S") && flagS <= 0 && currentmove1 < 0.0 && ltp_stock[timestep] >= 20.0 && shorttrades <= SC && flag1<0)
                                    {
                                    ...
                                    if (z1 >= siglevel1 && np[timestep] != 1 && (mode == "A" || mode == "L") && flagL <= 0 && currentmove1 > 0.0 && ltp_stock[timestep] >= 20.0 && longtrades <= LC && flag2<0)
                                    {
