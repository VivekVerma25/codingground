...
list_main <- vector('list',NOS)

names(list_main) <- list("PMN01","PMN02","PMN03","PMN05","PMN06","PMN07","PMN08","PMN11","PMN12","PMN13","PMN14","PMN15", "PMN16" ,"PMN17" ,"PMN18","PMN21","PMN22","PMN23","PMN24")
SNAMES <- data.frame(names(list_main))
colnames(SNAMES) <- "MN WEIGHTS"
DATE_TEMP = stck_lvl[nrow(stck_lvl),1]
DATE = data.frame(DATE_TEMP)
for(t in 2:length(list_main))
{
...
#################################################################################################################################################################
list_full <- vector('list',3)
RAW_PRICE= data.frame(read.csv(paste(path_input,"CashDB/","px_last.csv",sep="")))
RAW_DVOL= data.frame(read.csv(paste(path_input,"CashDB/","total_india_delivery_volume.csv",sep="")))
...
for(l in 1:length(lookback_period)){
  ...
  list_full[[l]] <- final_fac
}

#######################################################################################################################################################

fac_list = list_full
stapp = data.frame(read.csv(paste(path_input,"UNIVERSE/","ALLFOMINUSNIFTY.csv",sep="")))
...
for(z in 1 : 3)
{
  ...
  if(dsort!=1){
    ...
    #rite.csv(ret_fin_full,paste(path_output,names(fac_list)[z],"_FULL.csv"),row.names=FALSE)
    ################################################################################################################################
...
for(z in 1 : length(fac_list))
{
  ...
  stapp = data.frame(read.csv(paste(path_input,"UNIVERSE/","TOP50ANALYSTNIFTY50.csv",sep="")))
  ...
  WEIGHT_OP[z+3,] = wts_int_full[(dim(wts_int_full)[1]),]
  #write.csv(ret_fin_full, file = "ret_fin_full.csv")
...
for(z in 1 : length(fac_list))
{
  ...
  if(dsort!=1){
    ...
    if (topbot==2){       
      ...
      ret_fin_tb<-data.frame(cbind(stck_lvl$Date[c(-1,-2)],tot_ret_tb,lng_ret_tb,sht_ret_tb,wts_ini_full_tb[c(-1,-1*dim(wts_ini_full_tb)[1]),]))
      WEIGHT_OP[z+4,] = wts_ini_full_tb[(dim(wts_ini_full_tb)[1]-1),]
...
rownames(FINAL_FILE) <- names(list_main)
#rownames(WEIGHT_OP) <- names(list_full)
#write.csv(t(WEIGHT_OP),paste(path_output,"MN ALLOC.csv"))
write.csv(t(FINAL_FILE),paste(path_output,"MN ALLOC 26-05-2017_FULL.csv"))
...
#rownames(WEIGHT_OP) <- names(list_full)
#write.csv(t(WEIGHT_OP),paste(path_output,"MN ALLOC.csv"))
write.csv(t(FINAL_FILE),paste(path_output,"PMN04 26-05-2017.csv"))
