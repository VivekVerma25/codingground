...
{
    public class contra_spread1 : BasicStrategy
    {
        ...
        public object Lookback3 = 60;
        public object Lookback4 = 300;
        ...
        public contra_spread1(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            double corcoef = Convert.ToDouble(correlation); 
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                List<double> Move4 = new List<double>();
                ...
                for (int timestep = 1; timestep < (len); timestep++)
                {
                    ...
                    if (data.InputData[i].Dates[timestep].Date != data.InputData[i].Dates[timestep - 1].Date)
                    {
                        ...
                        //shorttrades = 0;
                        longtrades = 0;
                        if (Move1.Count() > lbk2 && Move3.Count() > lbk4 && Move4.Count() > lbk4)
                        {
                            series1 = Move1.ToArray();
                            newseries1 = UF.GetRange(series1, series1.Length - lbk2, series1.Length - 1);
                            
                            series3 = Move3.ToArray();
                            newseries3 = UF.GetRange(series3, series3.Length - lbk4, series3.Length - 1);
                            series4 = Move4.ToArray();
                            newseries4 = UF.GetRange(series4, series4.Length - lbk4, series4.Length - 1);
                            ...
                            corr = UF.Correlation(newseries3, newseries4);
                    ...
                    if (timestep - lbk > 0 && data.InputData[i].Dates[timestep].Date == data.InputData[i].Dates[timestep - lbk].Date)
                    {
                        ...
                        if (data.InputData[i].Dates[timestep].TimeOfDay >= DataRefStartTime && data.InputData[i].Dates[timestep].TimeOfDay <= DataRefEndTime)
                        {
                            ...
                            double currentmove3 = Math.Log(ltp_stock[timestep] / ltp_stock[timestep - 1]);
                            double currentmove4 = Math.Log(ltp_sec[timestep] / ltp_sec[timestep - 1]);

                            Move1.Add(Math.Abs(currentmove1));
                            Move2.Add(Math.Abs(currentmove2));
                            Move3.Add(Math.Abs(currentmove3));
                            Move4.Add(Math.Abs(currentmove4));


                            if (series1.Length > lbk2 && series3.Length > lbk4 && series4.Length > lbk4)
                            {
                                ...
                                if (data.InputData[i].Dates[timestep].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[timestep].TimeOfDay < TrdEntryEndTime)
                                {
                                    ...
                                    if (z1 >= siglevel1 && np[timestep - 1] != 1 && (mode == "A" || mode == "L") && flag <= 0 && currentmove1 < 0.0 && ltp_stock[timestep] >= 20.0 && corr>=corcoef)
                                    {
                    ...
                    if (((timeintrade >= lbk && timeintrade >= et1) || timeintrade >= et2) && np[timestep - 1] != 0)
                    {
