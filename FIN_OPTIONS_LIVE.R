...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/FINAL FILES/OUTPUT/OPTIONS TWAP NEW/"

stck_lvl<-data.frame(read.csv(paste(path_input,"FUT DB/","rollAdjustedTWAP0930_1030_final_new.csv",sep="")))
...
for(z in 1 : length(fac_list))
{
  ...
  if(dsort!=1){
    ...
    write.csv(ret_fin_full,paste(path_output,names(fac_list)[z],"_FULL.csv"),sep="",row.names=FALSE)
    ################################################################################################################################
    ...
    if (topbot==2){       
      ...
      write.csv(ret_fin_tb,paste(path_output,names(fac_list)[z],"_TOPBOT.csv"),sep="",row.names=FALSE)
