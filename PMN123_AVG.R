...
path_output<-"D:/LFT/Template/tester/R test data/full simulation/FINAL FILES/OUTPUT/PMN123_AVG/"
...
######################################################################################################################################################
RAW_PRICE= data.frame(read.csv(paste(path_input,"CASH DB/","px_last.csv",sep="")))
...
for(l in 1:length(lookback_period)){
  ...
  list_PDV[[l]] <- final_fac
}

names(list_PDV) <- list("PMN01","PMN02","PMN03")
...
for(j in 1 : length(list_PDV))
{
  
  fac_1<-list_PDV[[j]] # factor file input
  ...
  list_PDV[[j]] =  data.frame(fac_1[,1],rnk)
  
}

fac_list = vector('list',1)
names(fac_list) <- list("PMN123 AVG")

fac_list[[1]] = data.frame(fac_1[,1],(list_PDV[[1]][,-1] + list_PDV[[2]][,-1]+ list_PDV[[3]][,-1])/3)
