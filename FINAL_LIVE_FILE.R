...
stck_lvl<-data.frame(read.csv(paste(path_input,"FutDB/","RollAdjRet.csv",sep=""),header=FALSE))
xx = as.matrix(stck_lvl[1,])
stck_lvl<-data.frame(read.csv(paste(path_input,"FutDB/","RollAdjRet.csv",sep="")))
colnames(stck_lvl) =xx
...
#################################################################################################################################################################
  #raw_cash_close=data.frame((read.csv(paste(path_input,"CashDB/","px_last.csv",sep=""),header=FALSE)))
  #colnames(raw_cash_close) = as.vector(as.character(raw_cash_close[1,]))
  #x = as.matrix(raw_cash_close[1,])
  #colnames(raw_cash_close) = paste(raw_cash_close[1,],sep="",collapse =NULL)
  raw_cash_close=data.frame(read.csv(paste(path_input,"CashDB/","px_last.csv",sep="")))
  colnames(raw_cash_close) = xx
  #raw_cash_close = as.numeric(as.character(data.frame((raw_cash_close[-1,]),row.names = NULL)))
  
  raw_fut_close= data.frame(read.csv(paste(path_input,"FutDB/","px_last.csv",sep="")))
  colnames(raw_fut_close) = xx
  #raw_fut_close = data.frame(raw_fut_close[-1,],row.names = NULL)
...
colnames(RAW_BEST_AR) = xx
#RAW_BEST_AR = (RAW_BEST_AR[-1,])

RAW_BEST_TP= data.frame(read.csv(paste(path_input,"AnalystDB/","BEST_TARGET_PRICE.csv",sep="")))
colnames(RAW_BEST_TP) = xx
#RAW_BEST_TP = (RAW_BEST_TP[-1,])
...
write.csv(t(FINAL_FILE),paste(path_output,"MN ALLOC_NAMETEST.csv"))
