...
{
    public class NSE_AD_range_6tick_exit : BasicStrategy
    {
        ...
        public NSE_AD_range_6tick_exit(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    if ((np[j - 1] == 1 && diff2< -Math.Min(Math.Max(adm * newseries2.Average() / 2.5, 0.1), adm / 5)) || (np[j - 1] == -1 && diff2 > Math.Min(Math.Max(adm * newseries2.Average() / 2.5, 0.1), adm / 5)))
                    {
                        sig[j] = -np[j - 1];
                        np[j] = 0;
                    }
