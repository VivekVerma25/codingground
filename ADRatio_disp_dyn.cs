...
{
    public class ADRatio_disp_dyn : BasicStrategy
    {
        ...
        public ADRatio_disp_dyn(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    {
                        if ((diff1 / dist1) > Math.Min(dthresh * (75/(timecounter))/21, 0.7) && longflag == true)
                        {
                            sig[j] = +2;
                            np[j] = +1;
                            //openad2 = ad[j];
                        ...
                        if ((diff1 / dist1) < -Math.Min(dthresh * (75 / (timecounter)) / 21, 0.7) && shortflag == true)
                        {
                            sig[j] = -2;
                            np[j] = -1;
                            // openad2 = ad[j];
