...
{
    public class NSE_AD_range_final : BasicStrategy
    {
        ...
        public NSE_AD_range_final(string stratName, double alloc, double cost, double timeStep)
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime)
                    {
                        ...
                        {
                            if (diff1 > Math.Min(Math.Max(adm * ((timecounter)/ 75), 0.05), 0.5) && longflag == true)
                            {
                                sig[j] = +2;
                                np[j] = +1;
                            }

                            if (diff1 < -Math.Min(Math.Max(adm * ((timecounter) / 75), 0.05), 0.5) && shortflag == true)
                            {
                                sig[j] = -2;
                                np[j] = -1;
                            }
