...
{
    public class TMAwithStdevBand_Exit : BasicStrategy
    {
        ...
        public TMAwithStdevBand_Exit(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = tmaP; j < tma.Length; j++)
                {
                    ...
                    else if ((np[j - 1] == 1 && ltp[j] > uband[j]
                            && ltp[j - 1] < uband[j - 1]) || (np[j - 1] == -1 && ltp[j] < lband[j]
                            && ltp[j - 1] > lband[j - 1]))
                    {
                        sig[j] = -np[j - 1];
                        np[j] = 0;
