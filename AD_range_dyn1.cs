            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    if (data.InputData[i].Dates[j].TimeOfDay >= TrdEntryStartTime && data.InputData[i].Dates[j].TimeOfDay <= TrdEntryEndTime)
                    {
                        ...
                        {
                            if (diff1 > Math.Min(Math.Max(adm * (timecounter / 75) * newseries2.Average(), 0.25), 0.75) && longflag == true)
                            {
                                sig[j] = +2;
                                np[j] = +1;
                                openad2 = ad[j];
                            ...
                            if (diff1 < -Math.Min(Math.Max(adm * (timecounter / 75) * newseries2.Average(), 0.25), 0.75) && shortflag == true)
                            {
                                sig[j] = -2;
                                np[j] = -1;
                                openad2 = ad[j];
                    ...
                    if ((np[j - 1] == 1 && diff3< -Math.Min(Math.Max(adm * newseries2.Average() / 5, 0.1), adm / 2.5)) || (np[j - 1] == -1 && diff3 > Math.Min(Math.Max(adm * newseries2.Average() / 5, 0.1), adm / 2.5)))
                    {
                        sig[j] = -np[j - 1];
                        np[j] = 0;
                    }
