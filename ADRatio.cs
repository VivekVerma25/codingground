...
{
    public class ADRatio : BasicStrategy
    {
        ...
        public ADRatio(string stratName, double alloc, double cost, double timeStep)
        ...
        public override void RunStrategy(StrategyData data)
        {
            ...
            for (int i = 0; i < numSec; i++)
            {
                ...
                for (int j = (lag + 1); j < (ltp.Length - 1); j++)
                {
                    ...
                    if (timecounter == 1)
                    {
                        if (diff > adm && longflag == true && move < gap)
                        {
                            sig[j] = +2;
                            np[j] = +1;
                        }

                        if (diff < -adm && shortflag == true && move > -gap )
                        {
                            sig[j] = -2;
                            np[j] = -1;
                        }
